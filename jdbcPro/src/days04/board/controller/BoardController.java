...
public class BoardController {
	...
	private void 메뉴처리() {
		...
		switch (selectedMenu){
		case NEW: 새글쓰기();   break;
		case LIST: 목록보기(); break;
		case VIEW: 상세보기(); break;
		case EDIT: 수정하기();  break;
		case DELETE: 삭제하기();  break;
		case SEARCH: 검색하기();  break;
		case EXIT: 종료하기();  break;
		}//switch
	...
	private void 삭제하기() {
	    ...
	    boolean result = this.boardService.deleteService(seq);
	...
	private void 상세보기() {
		...
		System.out.print("> 보고자 하는 게시글 번호(seq)를 입력 ?");
	...
	private void 목록보기() {
		...
		//    ㄴ 목록 BoardService.목록보기()  /두번째생성
		//        ㄴ 목록 BoardDao.목록보기()  /첫번째 생성
		//결과물 -> 뷰(View)   // 세번째 생성

		System.out.println("> 현재 페이지번호 입력 : ");
		this.currentPage = this.scanner.nextInt();

		List<BoardDTO> list = this.boardService.selectService();


		// 뷰(View)역할도 컨트롤러에서 담당하자.		

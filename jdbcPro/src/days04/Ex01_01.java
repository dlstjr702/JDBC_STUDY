...
public class Ex01_01 {
	...
	public static void main(String[] args) {   
		...
		SalgradeVO sVo = null;
		ArrayList<EmpDeptSalgradeVO> edsList = null;
		EmpDeptSalgradeVO edsVo = null;
			...
			if (rs.next()) {
				...
				do {
					...
					try (
						...
						if ( eRs.next()) {
							edsList = new ArrayList<EmpDeptSalgradeVO>();
							do {
								...
								edsVo = EmpDeptSalgradeVO.builder()
										.deptno(deptno)
										.dname(dname)
										.empno(empno)
										.ename(ename)
	...
	private static void dispGradeEmp(LinkedHashMap<SalgradeVO, ArrayList<EmpDeptSalgradeVO>> map) {
		map.forEach((sVo,edsList)->{ 
			System.out.printf("%d등급   ( %d ~ %d ) - %d명\n",  sVo.getGrade(), sVo.getLosal(), sVo.getHisal(), sVo.getEmpCount());
			edsList.forEach(edsVo ->{
				  System.out.printf("   %d\t%s\t%d\t%s\t%.2f\n", edsVo.getDeptno(), edsVo.getDname(), edsVo.getEmpno(), edsVo.getEname(), edsVo.getSal());
			});
		});
